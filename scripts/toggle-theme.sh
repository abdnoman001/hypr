...
# 4a. Update Brave / Chromium policy theme colors
BRAVE_POLICY_DIR="/etc/brave/policies/managed"
if [ -d "$BRAVE_POLICY_DIR" ]; then
    echo "{\"BrowserThemeColor\": \"$BACKGROUND\", \"BrowserColorScheme\": \"device\"}" > "$BRAVE_POLICY_DIR/color.json"
    if pgrep -x brave >/dev/null; then
        brave --refresh-platform-policy --no-startup-window &>/dev/null &
    fi
fi

# 4b. Terminate running Nautilus instances to reload GTK CSS changes next time it starts
if pgrep -x nautilus >/dev/null; then
    nautilus -q &>/dev/null || killall -9 nautilus 2>/dev/null
fi

# 4c. Update Walker stylesheet variables dynamically
mkdir -p "$HOME/.config/walker"
cat <<EOF > "$HOME/.config/walker/theme.css"
...
# 8. Update Hyprlock (Modern Lock Screen) configuration
HYPRLOCK_CONFIG="$HOME/.config/hypr/hyprlock.conf"
mkdir -p "$HOME/.config/hypr"

cat <<EOF > "$HYPRLOCK_CONFIG"
# Automatically generated by toggle-theme.sh. Do not edit.
...
input-field {
    ...
    dots_center = true
    outer_color = rgba(${accent_raw}FF)
    inner_color = rgba(${bg_raw}EE)
    font_color = rgba(${fg_raw}FF)
    fade_on_empty = false
    placeholder_text = <span foreground="rgba(${fg_raw}99)">Enter Password...</span>
    hide_input = false
    position = 0, -80
    halign = center
    valign = center
